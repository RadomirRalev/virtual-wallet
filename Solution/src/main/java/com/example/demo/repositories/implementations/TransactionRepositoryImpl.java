...
import static com.example.demo.helpers.PaginationHelper.getPaginatedResult;
...
@Repository
@Transactional
public class TransactionRepositoryImpl implements TransactionFilterRepository {
    ...
    @Override
    public List<Transaction> getTransactionsByUserId(String direction, int userId, int page, String sort) {
        try (Session session = sessionFactory.openSession()) {
            List<Transaction> result = new ArrayList<>();
            if (direction.equalsIgnoreCase("All") || direction.equalsIgnoreCase("Outgoing")) {
                getQueryDirectionUserId(userId, session, result, FROM_INTERNAL_SENDERID);
                getQueryDirectionUserId(userId, session, result, FROM_WITHDRAWAL_SENDERID);
            }
            if (direction.equalsIgnoreCase("All") || direction.equalsIgnoreCase("Incoming")) {
                getQueryDirectionUserId(userId, session, result, FROM_INTERNAL_RECEIVERID);
                getQueryDirectionUserId(userId, session, result, FROM_DEPOSIT_RECEIVERID);
            }
            result = SortHelper.sortTransactions(result, sort);
            return getPaginatedResult(page, result);
        }
    }

    @Override
    public List<Transaction> getTransactionsByUserId(String direction, LocalDate startDate, LocalDate endDate, int userId, int page, String sort) {
        try (Session session = sessionFactory.openSession()) {
            List<Transaction> result = new ArrayList<>();
            if (direction.equalsIgnoreCase("All") || direction.equalsIgnoreCase("Outgoing")) {
    ...
    @Override
    public List<Transaction> getTransactionsByUserId(String direction, String recipientSearchString, int userId, int page, String sort) {
        try (Session session = sessionFactory.openSession()) {
            List<Transaction> result = new ArrayList<>();
    ...
    @Override
    public List<Transaction> getTransactionsByUserId(String direction, LocalDate startDate, LocalDate endDate, String recipientSearchString, int userId, int page, String sort) {
        try (Session session = sessionFactory.openSession()) {
            List<Transaction> result = new ArrayList<>();
            if (direction.equalsIgnoreCase("All")) {
                if (!recipientSearchString.equalsIgnoreCase(currentPrincipalName())) {
    ...
    private void getQueryDirectionUserId(int userId, Session session, List<Transaction> result, String query) {
        Query<Transaction> queryDirection = session.createQuery(query, Transaction.class);
        queryDirection.setParameter("userId", userId);
        result.addAll(queryDirection.list());
    }
